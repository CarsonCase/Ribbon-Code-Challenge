...
let mockToken: Contract, mockStrike: Contract, mockPriceOracle: Contract;
...
async function setupMockContracts(){
  ...
  // deploy mock contracts for local deployment to work
  const MockToken = await ethers.getContractFactory("MockToken");
  mockToken = await MockToken.deploy();
  mockStrike = await MockToken.deploy();
  await mockToken.deployed();
  await mockStrike.deployed();
  WETH_ADDRESS = mockToken.address;
  USDC_ADDRESS = mockStrike.address;
}
...
describe("RibbonThetaVault", async()=>{
  before(async()=>{
    await setupMockContracts();
  });

  describe("Setup Mock Contracts", async()=>{
    it("set up contracts",async()=>{
      expect(mockToken);
    })
  });

  it("runs Ribbon tests next", async()=>{

    // just do this to start. Replace all the constants with deployments
    behavesLikeRibbonOptionsVault({
      ...
      strikeAsset: mockStrike.address,
      collateralAsset: mockToken.address,
      chainlinkPricer: mockPriceOracle.address,
